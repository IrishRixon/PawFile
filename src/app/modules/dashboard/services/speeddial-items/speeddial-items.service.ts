...
export class SpeeddialItemsService {
  ...
  private toggleDialog: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false
  );
  toggleDialogObs: Observable<boolean> = this.toggleDialog.asObservable();

  private toggleAddDialog: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(false);
  toggleAddDialogObs: Observable<boolean> = this.toggleAddDialog.asObservable();

  private isPetMissing: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false
  );
      ...
      command: () => {
        if (this._id == '') {
          ...
          this.messageService.add({
            severity: 'error',
            summary: 'Select a Pet',
            detail: 'Please select a pet first.',
            life: 3000,
          });
        } else {
          const newVal = !this.isPetMissing.value;
          this.updateDetailsFormsService
            .updateMissing(`${this.urlRoot}/dashboard/updateMissing`, {
              isMissing: !this.isPetMissing.value,
              _id: this._id,
            })
            .subscribe({
              next: (res) => {
